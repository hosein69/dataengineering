...
export class NobitexProvider{
  constructor({apiBase='https://api.nobitex.ir',wsUrl='wss://wss.nobitex.ir/connection/websocket',timeoutMs=10000}={}){
  ...
  async getJson(path){
    ...
    const r=await fetch(url,{headers:{accept:'application/json','user-agent':'nobitex-copy-live/0.1'},signal:AbortSignal.timeout(this.timeoutMs)});
    if(!r.ok)throw new Error(`Nobitex HTTP ${r.status} for ${path}`);
    const d=await r.json();
    if(d?.status && d.status!=='ok')throw new Error(`Nobitex API status=${d.status}`);
    return d;
  }

  async orderbook(symbol){
    symbol=upper(symbol);
    const d=await this.getJson(`/v3/orderbook/${encodeURIComponent(symbol)}`);
    return this.normalizeOrderbook(symbol,d);
  }

  async trades(symbol){
    symbol=upper(symbol);
    const d=await this.getJson(`/v2/trades/${encodeURIComponent(symbol)}`);
    const rows=Array.isArray(d?.trades)?d.trades:[];
    return rows.map((t,i)=>({
      id:String(t.id??t.tradeId??`${symbol}:${t.time??i}:${t.price??''}`),
      symbol,
      time:t.time??t.timestamp??null,
      price:num(t.price),
      volume:num(t.volume??t.amount),
      type:String(t.type??t.side??'').toLowerCase()||null,
      raw:t
    }));
  }
  ...
  async snapshot(symbol,{tradeLimit=100,depth=20}={}){
    ...
    const [book,trades]=await Promise.all([this.orderbook(symbol),this.trades(symbol)]);
    const recent=trades.slice(0,tradeLimit);
    return {
      provider:'nobitex',
      mode:'rest-live',
      symbol:upper(symbol),
      ...
      book,
      trades:recent,
      metrics:this.metrics(book,recent,depth)
    };
  }
