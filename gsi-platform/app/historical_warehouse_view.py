# -*- coding: utf-8 -*-
"""GSI historical warehouse explorer.
    ...
    with t1:
        ...
        if runs.empty:
            ...
            with st.expander("Lineage سورس‌های آخرین Snapshot", expanded=False):
                ...
                else: st.dataframe(lin.drop(columns=["columns_json"],errors="ignore"),width="stretch",hide_index=True)
    ...
    with t2:
        ...
        if trend.empty:
            ...
            if x["numeric_value"].notna().any():
                st.line_chart(x.set_index("ref_date")[["numeric_value"]], width="stretch")
            st.dataframe(x[["ref_date","metric_label","numeric_value","unit","run_id"]], width="stretch", hide_index=True)
