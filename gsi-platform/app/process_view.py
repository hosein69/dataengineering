    ...
    with st.container(border=True):
        ...
        if timeline is not None and not timeline.empty:
            ...
            with st.expander("جزئیات مراحل و مبنای Deadline"):
                st.dataframe(t.drop(columns=["FX_CASE_KEY"], errors="ignore"), width="stretch", hide_index=True)
        ...
        if case_actions is not None and not case_actions.empty:
            ...
            if not az.empty:
                ...
                with st.expander("همه پیشنهادهای این پرونده"):
                    ...
                    st.dataframe(az[show], width="stretch", hide_index=True)
        ...
        with r1:
            ...
            if realloc is not None and not realloc.empty:
                ...
                if not rz.empty:
                    st.dataframe(rz, width="stretch", hide_index=True, height=220)
                else:
        ...
        with r2:
            ...
            if bridge is not None and not bridge.empty:
                ...
                if not bz.empty:
                    ...
                    st.dataframe(bz[show], width="stretch", hide_index=True, height=220)
                else:
        ...
        if legacy_signals is not None and not legacy_signals.empty:
            ...
            if not kz.empty:
                ...
                st.dataframe(kz[show], width="stretch", hide_index=True, height=260)
            else:
        ...
        if legacy_catalog is not None and not legacy_catalog.empty:
            with st.expander("کاتالوگ دانش منتقل‌شده و Provenance"):
                ...
                st.dataframe(legacy_catalog[cols], width="stretch", hide_index=True, height=320)
...
def render(extras: Dict, fdf: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        ...
        if variants is not None and not variants.empty:
            ...
            st.dataframe(
                v_show, width="stretch", hide_index=True, height=260,
                column_config={
    ...
    with c3, st.container(border=True):
        ...
        if conf is not None and not conf.empty:
            ...
            st.dataframe(conf, width="stretch", hide_index=True, height=220)
        else:
    ...
    with c4, st.container(border=True):
        ...
        if roots is not None and not roots.empty:
            ...
            st.dataframe(
                r, width="stretch", hide_index=True, height=300,
                column_config={
