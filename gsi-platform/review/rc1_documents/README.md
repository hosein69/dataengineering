# Build 29.7.7 RC1 — کاندیدای بازبینی Opus

این بسته تأیید Production ندارد. اصلاحات مرحله‌ای و شواهد تست در `CHANGELOG.md`، `TEST_AND_INVARIANTS.md` و `HANDOFF_FOR_OPUS.md` ثبت شده‌اند. وضعیت ACCEPT با بسته‌شدن Finding یکسان نیست؛ `FINDINGS_REGISTER.md` و `KNOWN_LIMITATIONS.md` مرجع محدودیت‌های جاری‌اند. رخداد خرابی index در یکی از DBهای تست هنوز علت‌یابی قطعی نشده و Gate انتشار فعال است.

راهنماها و نتایج نسخه‌های قبلی در ادامه و فایل‌های قدیمی، سوابق تاریخی‌اند؛ نتیجهٔ RC فقط در `review/validation` ثبت می‌شود.

---

# Build 29.7.6 — SAP Semantic DWH + Smart Chatbot + Streamlit Hardening

> **V29.7.6:** SAP به Grainهای مستقل `raw_rows / pr_items / workflow_rows / po_items` شکسته شد؛ PO موجودیت مستقل DWH شد؛ تاریخ‌های mixed SAP به ISO نرمال می‌شوند؛ Chatbot سؤال‌های شناسه‌محور را از Published Semantic DWH و سؤال‌های رویه‌ای را از Knowledge Base پاسخ می‌دهد؛ Streamlit API از `use_container_width` به `width` مهاجرت کرد. جزئیات: [SAP_SEMANTIC_DWH_CHATBOT_V29_7_6_FA.md](SAP_SEMANTIC_DWH_CHATBOT_V29_7_6_FA.md).

## مبنای نسخه 29.7.4

# Build 29.7.4 — جمعیت درجه‌اول کارشناسان + NTSW و زنجیره کامل Cash Flow

> **سیاست V29.7.4:** جمعیت گزارش اصلی از اجتماع فایل کارشناسان (`moghavemat`) و NTSW ساخته می‌شود. عباسی و SATA دیگر حق ایجاد پرونده جدید ندارند و فقط enrichment درجه‌دوم هستند. در انتخاب مقدار نیز همان سیاست V29.7.3 حفظ شده: کارشناسان/NTSW مقدم‌اند و tier پایین‌تر فقط خلأ را پر می‌کند.

Cash Flow نیز از Published Business DWH شروع می‌شود و زنجیره شواهد PI → درخواست تخصیص → تخصیص → تعهد → خرید ارز → تأمین وجه → پرداخت → رفع/برگشت تعهد → مانده تعهد را با Gap و Reconciliation صریح نمایش می‌دهد. هیچ Settlement یا جریان بانکی بدون شاهد ساخته نمی‌شود. جزئیات: [PRIMARY_POPULATION_CASHFLOW_CHAIN_V29_7_4_FA.md](PRIMARY_POPULATION_CASHFLOW_CHAIN_V29_7_4_FA.md).

راهنمای موتور و خروجی اختصاصی: [CASHFLOW_V29_7_0_FA.md](CASHFLOW_V29_7_0_FA.md).
اجرای مستقل: `streamlit run app/cashflow.py`. همین گزارش در Dashboard و تب خروجی Studio نیز موجود است.

# Build 29.6.10 — دیباگ فول‌استک

راهنمای این نسخه و محدودیت‌های اعتبارسنجی: [FULLSTACK_DEBUG_V29_6_10_FA.md](FULLSTACK_DEBUG_V29_6_10_FA.md).
برای اجرای تست‌ها: `pip install -r requirements-dev.txt` و سپس `python run_all_tests.py`.
نسخه برنامه را در Sidebar بررسی کنید. مطالب نسخه‌های قبل در ادامه تاریخی‌اند.
...
## V29.2 — Offline Knowledge Desk
برای شبکه بدون اینترنت، محیط «دستیار دانش بازرگانی» در Streamlit اجازه می‌دهد مسیر Shared Folder پایگاه دانش و مسیر آموزش‌ها را انتخاب کنید و با دکمه «ایجاد / به‌روزرسانی چت‌بات» ایندکس SQLite FTS5 و سرویس داخلی سبک ساخته شود. مسیر Share داخل HTML افشا نمی‌شود. جزئیات: `OFFLINE_KNOWLEDGE_DESK_V29_2_FA.md`.
