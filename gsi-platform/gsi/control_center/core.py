...
OPTIONAL = {'contacts': ('name','first_name','last_name','management','department','position','level','cluster_id','active'), 'clusters': ('active',), 'memberships': ('level','active','scope_employee_codes')}
...
def render_report(cfg, employee_code, cluster_id, report_kind='daily'):
    ...
    current=warehouse_context(employee_code,m,cfg)
    if current is None:
        require_encryption()
