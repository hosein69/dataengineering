...
def check_warehouse_writer():
    """Report writer state precisely; the lock file itself is never evidence of activity."""
    try:
        from .warehouse.store import Warehouse
        from .warehouse.writer_lock import WriterLock, WarehouseBusyError
        wh = Warehouse()
        lock_path = wh.path.with_suffix('.writer.lock')
        try:
            with WriterLock(lock_path, timeout=0.05, metadata={'operation':'doctor_probe'}):
                pass
        except WarehouseBusyError as ex:
            d = ex.as_dict()
            owner = ' / '.join(str(d.get(k)) for k in ('host','pid','run_id') if d.get(k)) or 'نامشخص'
            _say(WARN, f"نویسنده DWH هم‌اکنون فعال است ({owner}). این وضعیت خطای مسیر سورس نیست؛ اجرای دوم باید منتظر بماند.")
        else:
            _say(OK, "قفل نویسنده DWH آزاد است.")
    except Exception as ex:
        _say(WARN, f"وضعیت قفل نویسنده DWH قابل بررسی نبود: {ex}")
