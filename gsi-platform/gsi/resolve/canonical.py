...
اصلاحات:
  ...
  V29.7.3 فرمت مقدار هرگز حق عبور از tier بالاتر را ندارد.
"""
...
from ..config.authority import sort_source_candidates, source_tier
...
class CanonicalEntityResolver:
    ...
    def resolve(
        ...
        # سیاست سراسری گزارش: کارشناسان + NTSW درجه‌اول؛ عباسی + SATA درجه‌دوم.
        # merge order حق تغییر authority را ندارد. مرتب‌سازی stable است تا در
        # یک tier، ترتیب معنایی اعلام‌شده توسط همان فیلد حفظ شود.
        present = sort_source_candidates(present)
        ...
        for i in df.index:
            ...
            chosen = result[i]
            explanation = "مقدار سورس با بالاترین درجه اعتبار سراسری گزارش انتخاب شد."
            if attr_name == "شماره سفارش":
                # قاعده قدیمی «۸ رقمی مطلق» می‌توانست یک مقدار عباسی/SATA را
                # روی شاهد کارشناسان بنشاند. از V29.7.3 فرمت فقط داخل همان
                # authority tier برنده tie-break می‌کند و هرگز tier را رد نمی‌کند.
                top_tier = min(source_tier(v[1]) for v in vals)
                eight = [v[0] for v in vals
                         if source_tier(v[1]) == top_tier and _EIGHT_DIGIT.match(v[0])]
                if eight:
                    chosen = eight[0]
                    result[i] = chosen
                    explanation = (f"در همان درجه اعتبار برنده، قالب ۸ رقمی «{chosen}» "
                                   "به‌عنوان tie-break انتخاب شد؛ tier پایین‌تر مجاز به override نیست.")
