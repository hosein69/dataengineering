...
M1_MAIN = "M1 — جمعیت اصلی کارشناسان/NTSW"
M2_EXCLUDED = "M2 — فاقد منبع درجه‌اول، دارای ساتا/کوتاژ"
...
def partition(df: pd.DataFrame, moghavemat_available: bool = True) -> PartitionResult:
    ...
    in_ntsw = df.get("IS_IN_NTSW_PRIMARY", pd.Series(False, index=df.index)).fillna(False).astype(bool)
    in_primary = in_mogh | in_ntsw
    ...
    axis_b[~in_primary & has_evidence] = M2_EXCLUDED
    axis_b[in_primary] = M1_MAIN
    df[AXIS_B] = axis_b

    if not moghavemat_available and not in_ntsw.any():
        log.critical("🚨 هیچ‌یک از منابع درجه‌اول کارشناسان/NTSW در جمعیت موجود نیست؛ "
                     "محور پوشش معتبر نیست و فقط شواهد عملیاتی برای تعیین تکلیف باقی می‌ماند.")
        axis_b = pd.Series(M3_TO_RESOLVE, index=df.index, dtype="object")
        axis_b[has_evidence] = M2_EXCLUDED
        df[AXIS_B] = axis_b
