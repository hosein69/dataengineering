...
class NtswAdapter(SourceAdapter):
    ...
    def transform(self, sheets: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        if df_l is not None and not df_l.empty:
            ...
            lic[KEY_ORDER] = lic[order_col].map(clean_key) if order_col is not None else ""
        ...
        if df_c is not None and not df_c.empty:
            ...
            invalid |= c.groupby(KEY_REG)[p('CURRENCY')].transform('nunique').gt(1)
            invalid |= c[KEY_REG].map(clean_key).eq('') | c[p('CURRENCY')].map(clean_key).eq('')
    ...
    def _allocation_request_ledger(self, a: pd.DataFrame) -> pd.DataFrame:
        ...
        def state(row: pd.Series) -> str:
            ...
            if any(k in text for k in _REJECTED_STATES):
                return "REJECTED"
            if any(k in text for k in ("نشده", "عدم", "not approved")):
                return "OPEN"
    ...
    def _agg_allocation(self, a: pd.DataFrame) -> pd.DataFrame:
        ...
        for reg, g in a.groupby(KEY_REG, sort=False):
            ...
            def amount(frame: pd.DataFrame) -> float:
                if frame.empty:
                    return 0.0
                currencies = {clean_key(v) for v in g[p("REQ_CURRENCY")]}
                values = pd.to_numeric(frame[p("REQ_AMOUNT")], errors="coerce")
                if len(currencies) != 1 or not next(iter(currencies), "") or values.isna().any():
