...
``REQUIRED_CONTRACTS`` ثبت می‌کند. ``python -m gsi.doctor`` پیش از هر اجرا
...
#: کلید = مسیر ماژول نسبت به gsi ، مقدار = حداقل __contract__
REQUIRED_CONTRACTS: Dict[str, int] = {
    "config.sources": 3,      # SourceSpec.frame_map()  ← علت خطای AttributeError
    "adapters.base": 5,       # KEY_REG_FILE + KEY_MATERIAL + کلیدهای استاندارد
    "adapters.a50_ntsw": 3,   # Import Licence hub + Request-level Allocation Ledger
...
CONTRACT_NOTES: Dict[str, str] = {
    ...
    "adapters.base": "کلید مستقل KEY_REG_FILE در کنار REG/MATERIAL/PR و قرارداد adapterها",
    "adapters.a50_ntsw": "Import Licence hub برای REG_FILE + دفترچه درخواست‌محور تخصیص",
    "core.text": "توابع clean_order_ref و order_ref_base — برای مرجع سفارش مقاومت",
    "core.jalali": "موتور تقویم (نام قبلی calendar.py بود)",
    "dataio.merge": "safe_merge با گارد انفجار سطر + policy برای known-incomplete zero-match",
...
class ContractIssue:
    ...
    def message(self) -> str:
        where = f"gsi/{self.module.replace('.', '/')}.py"
...
def check_contracts() -> List[ContractIssue]:
    ...
    for mod_path, required in REQUIRED_CONTRACTS.items():
        ...
        try:
            mod = importlib.import_module(f"gsi.{mod_path}")
