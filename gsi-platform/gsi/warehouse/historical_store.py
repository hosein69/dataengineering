...
class Warehouse:
    ...
    def ensure_schema(self) -> None:
        ...
        """
        with _LOCK, self.transaction() as con:
    ...
    def _persist_kpis(self, con: sqlite3.Connection, rid: str, ref: str,
        ...
        if "مانده تعهد" in df.columns and safe_agg is not None:
            ...
            if has_fin_contract:
                from ..report.financial_summary import commitment_numeric_if_single_currency
                value, currency, display = commitment_numeric_if_single_currency(df)
                metrics.append(("commitment_balance", "مانده تعهد", value, display, currency or "multi/unknown"))
            else:
                # Backward compatibility for legacy snapshots that genuinely lack
                # currency metadata; still REG-grain safe, but unit cannot be proven.
...
def _warehouse_pointer_file() -> Path:
    ...
    aibl_home = os.environ.get("AIBL_HOME", "").strip()
    if gsi_home:
        home = Path(gsi_home).expanduser(); name = "historical_warehouse.path"
    elif aibl_home:
        home = Path(aibl_home).expanduser(); name = "warehouse.path"
    else:
        home = Path.home() / ".gsi"; name = "historical_warehouse.path"
...
def warehouse_from_settings() -> Warehouse:
    ...
    """
    explicit = (os.environ.get("GSI_HISTORICAL_WAREHOUSE_PATH", "") or os.environ.get("AIBL_WAREHOUSE_PATH", "")).strip()
    ...
    if chosen is None and pointer.exists():
        try:
            ...
            chosen = None
    if chosen is None:
        configured = getattr(SETTINGS, "WAREHOUSE_PATH", "") or os.path.join(getattr(SETTINGS, "daily_report_root", str(Path.home()/".gsi")), "GSI_historical_warehouse.sqlite3")
