...
DERIVED = {
    ...
    # مرجع ارزش فاکتور: **ساتا و NTSW** (تصمیم مالک، ۱۴۰۵/۰۷/۰۴).
    # ترخیص و کوتاژ عمداً حذف شدند: روی داده واقعی تا ۵۷ برابر با ساتا اختلاف
    # داشتند. پرکردن یک خانه خالی با عددی که مرجع نیست، همان «عدد غلط در
    # تصمیم» است با ظاهر بهتر. آن دو ستون از سیستم حذف نشده‌اند — هنوز در
    # CROSS_SOURCE مقایسه می‌شوند تا اختلاف دیده شود.
    "INVOICE_VALUE":      (["SATA_INVOICE_VALUE"], 0, True),
