# -*- coding: utf-8 -*-
"""Pure filtering logic for GSI Studio."""
...
def apply_filters(df: pd.DataFrame, state: FilterState) -> pd.DataFrame:
    ...
    if state.search.strip():
        ...
        if cols:
            ...
            mask = hay.str.contains(needle, regex=False, na=False)
            # Material is TEXT. Normalize both the query and KEY_MATERIAL so searches
            # with letters, Persian digits, spaces or hyphens resolve consistently.
            if "KEY_MATERIAL" in out.columns:
                from ..core.text import clean_part_no
                material_needle = clean_part_no(raw_needle)
                if material_needle:
                    material = out["KEY_MATERIAL"].fillna("").map(clean_part_no)
                    mask |= material.str.contains(material_needle, regex=False, na=False)
            out = out[mask]
    return out
