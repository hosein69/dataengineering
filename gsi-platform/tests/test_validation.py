...
def test_pipeline():
    ...
    check("محور B: جمعیت درجه‌اول کارشناسان/NTSW در گزارش اصلی باقی ماند (V29.7.4)",
          len(res.main) >= 1 and (df["PARTITION_COVERAGE"].astype(str).str.contains("کارشناسان/NTSW").sum() >= 1),
          f"main={len(res.main)} / to_resolve={len(res.to_resolve)}")
...
def test_excel(res) -> None:
    ...
    # جمع native در این شیت عمداً نباید با SUBTOTAL(109) روی ردیف‌های
    # BL×Material محاسبه شود؛ آن الگو هم fan-out می‌دهد و هم ارزهای متفاوت
    # را بدون واحد با هم جمع می‌زند. قرارداد فعلی، نمایش تصمیم‌ساز REG/ارز است.
    has_safe_native_label = any(
        c.value == "جمع Native قابل اتکا (REG/ارز):"
        for row in wsc.iter_rows() for c in row
    )
    unsafe_native_subtotal = any(
        isinstance(c.value, str) and c.value.upper().startswith("=SUBTOTAL(109")
        for row in wsc.iter_rows() for c in row
    )
    check("جمع تعهد تصمیم‌ساز بدون SUBTOTAL چندارزی و با نمایش REG/ارز",
          has_safe_native_label and not unsafe_native_subtotal)
    ...
    check("گزارش ممیزی تعارضات ساخته شد",
          os.path.exists(os.path.join(os.path.dirname(res.dashboard_path), "GSI_Data_Conflicts_Audit.xlsx")))
