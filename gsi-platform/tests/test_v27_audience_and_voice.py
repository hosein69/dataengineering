...
def test_html_respects_audience() -> None:
    ...
    check("هر فایل فقط یک مخاطب دارد",
          all(h.count('class="chip persona"') == 1 for h in pages.values()))
    check("فقط مخاطب انتخاب‌شده در metadata وجود دارد",
          all('const AUDIENCES={"'+k+'":' in h for k,h in pages.items()))
    ...
    check("بخش‌ها با data-section علامت خورده‌اند تا جابه‌جایی کار کند",
          all("data-composer-block" in h for h in pages.values()))
    check("اندازه صفحه از پروفایل می‌آید، نه عدد ثابت",
          all("audCfg().table_rows" in h for h in pages.values())
          and all("const PAGE_SIZE=pageSize()" in h for h in pages.values()))
