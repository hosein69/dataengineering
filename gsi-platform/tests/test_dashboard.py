...
def test_metrics() -> None:
    ...
    check("جمع بدون کلید ثبت سفارش و ارز ساخته نمی‌شود",
          "نیازمند" in by["جمع مانده تعهد"]["value"],
          str(by["جمع مانده تعهد"]["value"]))
...
def test_email_from_hr() -> None:
    ...
    try:
        ...
        rec: list = []
        _h = _LogCatcher(rec)
        _previous_level = de.log.level
        de.log.setLevel(logging.INFO)
        de.log.addHandler(_h)
        try:
            de.hr_recipients(ppl)
        finally:
            de.log.removeHandler(_h)
            de.log.setLevel(_previous_level)
        body = " ".join(rec)
