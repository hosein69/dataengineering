...
def discover_suites():
    """Retain legacy entrypoints, and include every newer test file."""
    named = {path: label for label, path in SUITES}
    return [(named.get(path, Path(path).stem), path)
            for path in sorted(p.relative_to(ROOT).as_posix()
                               for p in list(Path(ROOT, "tests").glob("test_*.py")) + list(Path(ROOT, "tests").glob("legacy_test_*.py")))]


def test_command(path):
    tree = ast.parse(Path(ROOT, path).read_text(encoding="utf-8"))
    standalone = any(isinstance(node, ast.If) and "__name__" in ast.unparse(node.test)
                     and "__main__" in ast.unparse(node.test) for node in tree.body)
    return [sys.executable, path] if standalone else [sys.executable, "-m", "pytest", "-q", path]
...
def main() -> int:
    ...
    failed_suites = []
    timed_out = []
    for label, path in discover_suites():
        print("\n" + "\u2588" * 78)
        print(f"\u2588  {label}")
        print("\u2588" * 78)
        ...
        if not line:
            ...
            if passed:
                total_ok += int(passed.group(1))
            elif ran and out.returncode == 0:
                total_ok += int(ran.group(1))
            if out.returncode != 0:
                total_fail += 1
        if out.returncode != 0:
    ...
    if failed_suites:
        ...
        print("✅ همه مجموعه‌های تست اجراشده موفق بودند؛ اعتبارسنجی داده واقعی جداگانه لازم است.")
    print("═" * 78)
