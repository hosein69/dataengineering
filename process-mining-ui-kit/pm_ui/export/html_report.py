...
EMAIL_FONT_STACK = "'IRANSans','Tahoma','Segoe UI',Arial,sans-serif"
...
<td width="25%" style="padding:6px" valign="top">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
    ...
    <tr><td style="padding:14px 16px">
      ...
      <div style="font-size:22px;font-weight:800;color:{T.INK};margin-top:6px">{_esc(k.get('value',''))}</div>
...
def top_routes_table_html(nodes: Sequence[Mapping], edges: Sequence[Mapping], *,
                           top_n: int = 6) -> str:
    """به‌جای گراف SVG تعاملی: جدول مسیرهای پرتکرار فرآیند (ایمن برای Outlook)."""
    labels = {n.get("id"): n.get("label", n.get("id")) for n in nodes}
    total = sum(max(0, int(e.get("count", 0))) for e in edges) or 1
    ranked = sorted(edges, key=lambda e: -int(e.get("count", 0)))[:top_n]
    rows = []
    for e in ranked:
        count = int(e.get("count", 0))
        share = round(100 * count / total)
        critical = bool(e.get("critical"))
        color = T.STATUS["critical"].ink if critical else T.INK
        mark = "⬤ " if critical else ""
        route = f"{labels.get(e.get('source'), e.get('source'))} ← {labels.get(e.get('target'), e.get('target'))}"
        rows.append(f"""
<tr>
  <td style="padding:8px 10px;border-bottom:1px solid {T.BORDER};color:{color};font-weight:600">{mark}{_esc(route)}</td>
  <td style="padding:8px 10px;border-bottom:1px solid {T.BORDER};text-align:center;color:{T.INK}">{_esc(fa.fa_number(count))}</td>
  <td style="padding:8px 10px;border-bottom:1px solid {T.BORDER};text-align:center;color:{T.INK_MUTED}">{_esc(fa.to_fa_digits(share))}٪</td>
</tr>""")
    head = f"""
<tr style="background:{T.SURFACE_SUNKEN}">
  <th style="padding:8px 10px;text-align:right;font-size:12px;color:{T.INK_MUTED}">مسیر</th>
  <th style="padding:8px 10px;font-size:12px;color:{T.INK_MUTED}">تعداد کیس</th>
  <th style="padding:8px 10px;font-size:12px;color:{T.INK_MUTED}">سهم</th>
</tr>"""
...
<div style="background:{T.INK};color:#fff;border-radius:{T.RADIUS['lg']}px;padding:20px 24px">
  <div style="font-size:20px;font-weight:800">{_esc(title)}</div>
  {sub}
