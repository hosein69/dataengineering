...
def _legacy_key(key: str) -> str:
    """Migration alias: GSI_* first; legacy AIBL_* only as fallback."""
    return "AIBL_" + key[4:] if key.startswith("GSI_") else key
...
def _env_optional(key: str) -> Optional[str]:
    ...
    return os.environ.get(_legacy_key(key))
...
def _local_default(win_path: str, posix_leaf: str) -> str:
    ...
    زیر ``GSI_HOME`` (پیش‌فرض ``~/.gsi``) می‌نویسیم.
    """
    if _IS_WINDOWS:
        return win_path
    home = _env("GSI_HOME", os.path.join(os.path.expanduser("~"), ".gsi"))
...
class Settings:
    # ── ریشه‌های شبکه ──
    FOREIGN_DIR: str = field(default_factory=lambda: _env("GSI_FOREIGN", rf"{_NET}\03-Data\01-Foreign"))
    BLS_TOTAL_DIR: str = field(default_factory=lambda: _env("GSI_BLS", rf"{_NET}\03-Data\01-Foreign\BLs TOTAL"))
    CLEARANCE_DIR: str = field(default_factory=lambda: _env("GSI_CLEARANCE", rf"{_NET}\03-Data\01-Foreign\BLs TOTAL\Clearance"))
    HR_DIR: str = field(default_factory=lambda: _env("GSI_HR", rf"{_NET}\11-Governance & Integration\03-Reports\01-HR"))
    # مسیر واقعی فایل مقاومت (تأییدشده در HEADERS_MAP) — نه پوشه اسماعیلی
    GS_COMBINE_OUT_DIR: str = field(default_factory=lambda: _env("GSI_GS_COMBINE", rf"{_NET}\11-Governance & Integration\DataTeam\Data_Ware_House\GS_Combine\OUTPUT"))
    ESMAEILI_DIR: str = field(default_factory=lambda: _env("GSI_ESMAEILI", rf"{_NET}\11-Governance & Integration\25-H.Esmaeili"))
    MOHAMADI_DIR: str = field(default_factory=lambda: _env("GSI_MOHAMADI", rf"{_NET}\11-Governance & Integration\26-M.Mohamadi"))

    # ── خروجی و لاگ ──
    OUTPUT_DIR: str = field(default_factory=lambda: _env("GSI_OUTPUT", _local_default(r"D:\of\blstotal\output", "output")))
    LOG_DIR: str = field(default_factory=lambda: _env("GSI_LOGS", _local_default(r"D:\of\blstotal\logs", "logs")))
    # خروجی روزانه رسمی داشبورد: پوشه تاریخ‌دار + نام ثابت برای استفاده سازمانی/ایمیل
    DAILY_REPORT_ROOT: str = field(default_factory=lambda: _env("GSI_DAILY_REPORT_ROOT", ""))
    SYSTEMMATIC_MATERIAL_BASENAME: str = "Systemmatic Material.xlsx"

    # ── پارامترهای اجرا ──
    # FIX-4: تاریخ هاردکد "2026-07-26" حذف شد.
    # اگر GSI_TODAY ست شود (برای تست بازتولیدپذیر) همان استفاده می‌شود.
    TODAY_OVERRIDE: Optional[str] = field(default_factory=lambda: _env_optional("GSI_TODAY"))
