...
class CriticalityEngine:
    ...
    def evaluate(self, row: Dict[str, Any]) -> CriticalityResult:
        ...
        if not has_need or not complete:
            days = None
            ...
            days = round(min(float(total_confirmed or 0.0) / need, self.max_days), 1)
            band = (self._band(BAND_STOCKOUT) if float(total_confirmed or 0.0) <= 0
