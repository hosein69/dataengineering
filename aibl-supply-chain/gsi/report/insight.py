...
# ── پالت چهار لایه — مشتق از سیستم طراحی ───────────────────────────────────
#
# این فایل قبلاً پالت مستقل خودش را داشت (``0F6E6E`` / ``1E9E9E`` / …) و یکی
# از مقادیرش کف WCAG را رد می‌کرد:
#
#     لایه ۲: سفید روی ``1E9E9E``  →  ۳٫۲۶   (کف متن عادی ۴٫۵ است)
#
# یعنی سرستون لایه دوم روی مانیتور ارزان یا در نور روز خوانده نمی‌شد. حالا
# هر چهار لایه از :mod:`gsi.design.tokens` می‌آیند و ترتیب تأکیدشان هم حفظ
# شده: هرچه لایه عمیق‌تر، پس‌زمینه تیره‌تر و متن پرکنتراست‌تر.
#
#     لایه ۱  سفید روی سرمه‌ای برند   ۱۶٫۶۹
#     لایه ۲  سفید روی فیروزه‌ای متنی  ۶٫۶۸
#     لایه ۳  متن اصلی روی wash        ۱۴٫۵۲
#     لایه ۴  متن ثانویه روی sunken    ۷٫۲۹
from ..design import excel as _dx
from ..design import tokens as _T

AQUA_DEEP = _dx.X(_T.BRAND_NAVY)        # عنوان لایه ۱ — بیشترین تأکید
AQUA = _dx.X(_T.TEAL_INK)               # لایه ۲ (بود 1E9E9E با کنتراست ۳٫۲۶)
AQUA_SOFT = _dx.X(_T.SEQUENTIAL[2])     # فقط سطح؛ هرگز زیر متن سفید نمی‌رود
GREEN_SOFT = _dx.X(_T.TEAL_WASH)        # پس‌زمینه لایه ۳
GREY_TEXT = _dx.X(_T.TEXT_SECONDARY)    # لایه ۴ (محاسبات)
GREY_BG = _dx.X(_T.SURFACE_SUNKEN)
WHITE = _dx.X(_T.TEXT_ON_DARK)
INK_ON_WASH = _dx.X(_T.TEXT)            # متن روی لایه ۳
BROKEN_FILL = _dx.X(_T.STATUS["critical"].wash)   # حلقه پاره زنجیره
...
def _write_row(ws, r: int, level: int, label: str, counts: List[int],
    ...
    style = {
        ...
        3: (GREEN_SOFT, INK_ON_WASH, False, 11),
...
def _build_material(self, df: "pd.DataFrame") -> None:
    ...
    for mat, g in df[df[key].astype(str).str.strip() != ""].groupby(key, sort=False):
        ...
        for j, n in enumerate(counts, start=7):
            ...
            cell.fill = P.fill(GREEN_SOFT if n else BROKEN_FILL)
    ...
    s2.marker.graphicalProperties = GraphicalProperties(solidFill=_dx.X(_T.CATEGORICAL[3]))
