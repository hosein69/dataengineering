...
class CriticalityStage(Stage):
    name = "criticality"
    title = "محاسبه مقاومت قطعه (Oracle + موجودی کارشناسی ÷ مصرف روزانه)"
    order = 40
    ...
    def run(self, df: pd.DataFrame, ctx: PipelineContext) -> pd.DataFrame:
        ...
        if known == 0:
            log.critical(
                "🚨 مقاومت قطعی هیچ قطعه‌ای محاسبه نشد — اجزای موجودی کارشناسی/Oracle یا نیاز روزانه ناقص است.\n"
                "   ⇒ Unknown هرگز Zero فرض نمی‌شود؛ Supply Position / Data Lineage را بررسی کنید.")
        else:
    ...
    def math_docs(self, ctx: PipelineContext) -> Dict[str, Dict]:
        ...
        return {"مقاومت قطعه": {
            "فرمول": "مقاومت قطعی (روز) = (ایران‌خودرو Oracle + ساپکو Oracle + نزد سازنده کارشناس + در راه کارشناس + گمرک کارشناس) ÷ نیاز روزانه؛ فقط وقتی همه مؤلفه‌ها موجود باشند.",
            ...
            "قاعده داده": "سه مؤلفه نزد سازنده/در راه/گمرک از سورس کارشناسی‌اند؛ Oracle فقط ایران‌خودرو/ساپکو و نیاز روزانه را می‌دهد. Unknown ≠ Zero.",
