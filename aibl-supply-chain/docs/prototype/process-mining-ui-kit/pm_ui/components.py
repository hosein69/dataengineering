...
def kpi_row_html(kpis: Iterable[KpiSpec], *, columns: Optional[int] = None) -> str:
    """فقط نشانه‌گذاری+CSS ردیف KPI (بدون iframe) — برای استفاده در
    :func:`render_kpi_row` (Streamlit) و در خروجی HTML مستقل
    (:mod:`pm_ui.export.standalone_html`)، بدون تکرار نشانه‌گذاری."""
    items = list(kpis)
    ...
    return f"""
<div class="wrap kpi-wrap">
<style>
  .kpi-wrap{{display:grid;grid-template-columns:{col_css};gap:12px}}
  .kpi-wrap .kpi{{
    ...
    animation:pm-rise .45s cubic-bezier(.22,1,.36,1) both;
    animation-delay:calc(var(--i) * 55ms);
  }}
  .kpi-wrap .kpi:hover{{transform:translateY(-3px);box-shadow:{T.SHADOW_CARD_HOVER}}}
  .kpi-wrap .kpi .bar{{position:absolute;inset:0 0 auto 0;height:3px}}
  .kpi-wrap .kpi .top{{display:flex;align-items:center;justify-content:space-between}}
  .kpi-wrap .kpi .lab{{font-size:12.5px;color:{T.INK_MUTED};font-weight:600}}
  .kpi-wrap .kpi .ic{{margin-left:6px}}
  .kpi-wrap .kpi .val{{font-size:26px;font-weight:700;color:{T.INK};margin-top:8px;
    line-height:1.15;unicode-bidi:plaintext}}
  .kpi-wrap .kpi .foot{{display:flex;align-items:center;justify-content:space-between;margin-top:10px}}
  .kpi-wrap .kpi .d{{font-size:12px;font-weight:700}}
  .kpi-wrap .kpi .spark{{line-height:0}}
  @keyframes pm-rise{{from{{opacity:0;transform:translateY(8px)}}to{{opacity:1;transform:none}}}}
  @media (prefers-reduced-motion:reduce){{.kpi-wrap .kpi{{animation:none;transition:none}}}}
</style>
{cards}
</div>"""
...
def render_kpi_row(kpis: Iterable[KpiSpec], *, height: int = 168,
    ...
    items = list(kpis)
    html = f"<div style=\"font-family:{T.FONT_STACK};direction:rtl\">{kpi_row_html(items, columns=columns)}</div>"
...
def insight_row_html(cards: Iterable[InsightSpec], *, columns: Optional[int] = None) -> str:
    """فقط نشانه‌گذاری+CSS ردیف کارت شاهد/بینش (بدون iframe) — مشترک بین
    :func:`render_insight_row` و خروجی HTML مستقل."""
    items = list(cards)
    ...
    return f"""
<div class="wrap insight-wrap">
<style>
  .insight-wrap{{display:grid;grid-template-columns:{col_css};gap:12px}}
  .insight-wrap .insight{{
    ...
    animation:pm-rise2 .4s cubic-bezier(.22,1,.36,1) both;
    animation-delay:calc(var(--i) * 50ms);
    transition:box-shadow .25s, transform .25s;
  }}
  .insight-wrap .insight:hover{{transform:translateY(-2px);box-shadow:{T.SHADOW_CARD_HOVER}}}
  .insight-wrap .insight .ic{{font-size:40px;line-height:1;margin-bottom:{T.SPACE['xl']}px;color:{T.BRAND_TEAL}}}
  .insight-wrap .insight .head{{font-size:24px;font-weight:700;line-height:1.2;color:{T.INK};
    letter-spacing:-.48px;unicode-bidi:plaintext}}
  .insight-wrap .insight .body{{font-size:14px;font-weight:400;line-height:1.4;color:{T.INK_SOFT};
    margin-top:{T.SPACE['2xs']}px;unicode-bidi:plaintext}}
  @keyframes pm-rise2{{from{{opacity:0;transform:translateY(8px)}}to{{opacity:1;transform:none}}}}
  @media (prefers-reduced-motion:reduce){{.insight-wrap .insight{{animation:none;transition:none}}}}
</style>
{cards_html}
</div>"""
...
def render_insight_row(cards: Iterable[InsightSpec], *, height: int = 150,
    ...
    items = list(cards)
    html = f"<div style=\"font-family:{T.FONT_STACK};direction:rtl\">{insight_row_html(items, columns=columns)}</div>"
...
def evidence_table_html(spec: EvidenceTableSpec) -> str:
    """فقط نشانه‌گذاری+CSS جدول شواهد (بدون iframe) — مشترک بین
    :func:`render_evidence_table` و خروجی HTML مستقل."""
...
<style>
  ...
  .evtable h3{{margin:0;font-size:24px;font-weight:700;color:{T.INK}}}
  .evtable .sub{{font-size:14px;color:{T.INK_MUTED};margin-top:-{T.SPACE['sm']}px}}
  .evtable table{{width:100%;border-collapse:collapse;font-size:14px}}
  .evtable th{{background:{T.SURFACE_SUNKEN};color:{T.INK_SOFT};font-weight:700;font-size:13px;
    padding:10px 12px;text-align:center;border-bottom:1px solid {T.BORDER}}}
  .evtable td{{padding:12px;text-align:center;color:{T.INK};font-weight:600;font-size:15px;
    border-bottom:1px solid {T.BORDER};unicode-bidi:plaintext}}
  .evtable td.empty{{color:{T.INK_MUTED};font-weight:400;font-size:12.5px;padding:22px 12px}}
  .evtable .meta{{font-size:12.5px;color:{T.INK_MUTED}}}
  .evtable .gap{{font-size:15px;font-weight:700;color:{T.STATUS['warning'].ink};
    background:{T.STATUS['warning'].wash};border-radius:{T.RADIUS['sm']}px;
    padding:10px 14px}}
  .evtable .note{{font-size:13px;color:{T.INK_SOFT};line-height:1.7}}
  .evtable .actions{{display:flex;gap:{T.SPACE['lg']}px}}
  .evtable .action{{font-size:14px;font-weight:700;color:{T.BRAND_TEAL};cursor:default}}
</style>
...
def render_evidence_table(spec: EvidenceTableSpec, *, height: int = 420) -> None:
    """جدول «شواهد اول» را با ``components.v1.html`` رندر می‌کند.

    الگوی دقیق پنل «Settlement / Evidence comparison» در صفحهٔ فیگمای
    ``GSI / Cash Flow / Evidence-first``: هر عدد باید منبع، ارز/واحد، و
    زمان مشاهده داشته باشد؛ نبودِ یک سند هرگز به «انجام‌نشدن قطعی» ترجمه
    نمی‌شود — به همین دلیل ``gap_warning`` جدا از ``gap_note`` نمایش داده
    می‌شود: یکی ادعا، دیگری احتیاط در تفسیر.
    """
    import streamlit.components.v1 as components

    html = f"<div style=\"font-family:{T.FONT_STACK}\">{evidence_table_html(spec)}</div>"
    components.html(html, height=height, scrolling=True)


class ConformanceSpec(TypedDict, total=False):
    fitness_pct: float        # امتیاز انطباق کلی (۰..۱۰۰)
    total_cases: int
    deviating_cases: int


def _donut_svg(pct: float, color: str, *, size: int = 72) -> str:
    pct = max(0.0, min(100.0, pct))
    r = 15.9
    dash = f"{pct:.1f} {100 - pct:.1f}"
    return (f'<svg width="{size}" height="{size}" viewBox="0 0 42 42" role="img" aria-hidden="true">'
            f'<circle cx="21" cy="21" r="{r}" fill="none" stroke="{T.SURFACE_SUNKEN}" stroke-width="5"></circle>'
            f'<circle cx="21" cy="21" r="{r}" fill="none" stroke="{color}" stroke-width="5" '
            f'stroke-dasharray="{dash}" stroke-dashoffset="25" transform="rotate(-90 21 21)" '
            f'stroke-linecap="round"></circle></svg>')


def conformance_tile_html(spec: ConformanceSpec) -> str:
    """کاشی «امتیاز انطباق» — همان‌جایی که کاربر بدون رفتن به تب جداگانه
    می‌فهمد نقشهٔ جلوی چشمش چقدر قابل‌اتکاست (اصل GP2 مانیفست IEEE Task
    Force؛ یافتهٔ بخش ۱٫۳ بررسی UX). فقط نشانه‌گذاری+CSS (بدون iframe) —
    مشترک بین :func:`render_conformance_tile` و خروجی HTML مستقل."""
    pct = float(spec.get("fitness_pct", 0))
    status = T.STATUS["good"] if pct >= 80 else (T.STATUS["warning"] if pct >= 60 else T.STATUS["critical"])
    total = spec.get("total_cases", 0)
    deviating = spec.get("deviating_cases", 0)
    return f"""
<div class="wrap conf-tile" dir="rtl">
<style>
  .conf-tile{{display:flex;align-items:center;gap:14px;background:{T.SURFACE_SUNKEN};
    border-radius:{T.RADIUS['md']}px;padding:14px 16px}}
  .conf-tile b{{display:block;font-size:22px;font-weight:800;color:{T.INK}}}
  .conf-tile span{{font-size:11.5px;color:{T.INK_MUTED}}}
</style>
{_donut_svg(pct, status.fill)}
<div><b>{esc(f'{pct:.0f}٪')}</b>
<span>امتیاز انطباق کلی — {esc(fa.fa_number(deviating))} از {esc(fa.fa_number(total))} پروندهٔ دارای انحراف</span></div>
</div>"""


def render_conformance_tile(spec: ConformanceSpec, *, height: int = 100) -> None:
    """جدا از رفتن به یک تب مستقل — این کاشی کنار KPIها/نقشه می‌نشیند تا
    امتیاز انطباق همیشه هم‌زمان با خودِ نقشه دیده شود، نه پشتِ یک کلیک."""
    import streamlit.components.v1 as components

    html = f"<div style=\"font-family:{T.FONT_STACK}\">{conformance_tile_html(spec)}</div>"
    components.html(html, height=height, scrolling=False)


class RootCauseRow(TypedDict, total=False):
    dimension: str      # مثلاً «واحد گمرک — بندر جنوبی»
    delta_pct: float     # تفاضل امضادار نسبت به میانگین کل، مثلاً ‎+۱۸
    status: str           # کلید STATUS برای رنگ نوار — تفسیر شدت، نه ادعای علّیت


def root_cause_bars_html(rows: Sequence[RootCauseRow]) -> str:
    """ریشه‌یابی تفاضلی — نرخ انحراف هر بُعد نسبت به میانگین کل؛ فقط
    مشاهده، نه ادعای علّیت (همان مرز صریحی که در Pattern Observation
    محصول هم رعایت شده: «فقط مشاهده، نه Risk/SLA/Violation خودکار»)."""
    items = []
    max_abs = max((abs(float(r.get("delta_pct", 0))) for r in rows), default=1) or 1
    for r in rows:
        delta = float(r.get("delta_pct", 0))
        status = T.status_of(r.get("status") or ("critical" if delta > 10 else "warning" if delta > 0 else "good"))
        width = max(6, abs(delta) / max_abs * 100)
        sign = "‎+" if delta >= 0 else "‎−"
        items.append(f"""
<div class="rc-row">
  <span class="rc-dim">{esc(r.get('dimension',''))}</span>
  <div class="rc-track"><span style="width:{width:.0f}%;background:{status.fill}"></span></div>
  <span class="rc-delta" style="color:{status.ink}">{esc(f'{sign}{abs(delta):.0f}٪')}</span>
</div>""")
    return f"""
<div class="wrap rc-wrap" dir="rtl">
<style>
  .rc-wrap{{display:grid;gap:9px}}
  .rc-row{{display:grid;grid-template-columns:minmax(0,1fr) 90px 46px;gap:8px;align-items:center;font-size:11.5px}}
  .rc-dim{{color:{T.INK_SOFT};overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}
  .rc-track{{height:7px;background:{T.SURFACE_SUNKEN};border-radius:999px;overflow:hidden}}
  .rc-track>span{{display:block;height:100%;border-radius:999px}}
  .rc-delta{{font-weight:800;text-align:left;font-variant-numeric:tabular-nums}}
</style>
{"".join(items)}
</div>"""


def render_root_cause_bars(rows: Sequence[RootCauseRow], *, height: Optional[int] = None) -> None:
    import streamlit.components.v1 as components

    height = height or (40 + 32 * max(1, len(rows)))
    html = f"<div style=\"font-family:{T.FONT_STACK}\">{root_cause_bars_html(rows)}</div>"
    components.html(html, height=height, scrolling=False)
