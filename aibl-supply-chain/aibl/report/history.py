...
def snapshot(df: pd.DataFrame, ref_date: date | str) -> dict:
    ...
    late = pd.to_numeric(df.get("روزهای تأخیر", pd.Series(0, index=df.index)), errors="coerce").fillna(0)
    return {
        ...
        "مانده تعهد": round(safe_agg(df, "مانده تعهد", "sum"), 2) if "مانده تعهد" in df.columns else 0.0,
        "مانده تعهد معوق": round(safe_agg(df.loc[late > 0], "مانده تعهد", "sum"), 2) if "مانده تعهد" in df.columns else 0.0,
...
def write_history_sheet(wb, hist: pd.DataFrame) -> None:
    ...
    ws["A1"].font = Font(name="IRANSans", size=14, bold=True, color="FFFFFF")
    ws["A1"].fill = PatternFill("solid", fgColor="0A4F4F")
    cols = list(hist.columns)
    for j, c in enumerate(cols, 1):
        cell = ws.cell(3, j, c); cell.font = Font(name="IRANSans", bold=True, color="FFFFFF")
        cell.fill = PatternFill("solid", fgColor="406057"); cell.alignment = Alignment(horizontal="center")
    ...
    if len(hist) >= 2 and "مانده تعهد" in cols:
        chart = LineChart(); chart.title = "روند مانده تعهد"; chart.height = 7; chart.width = 14
