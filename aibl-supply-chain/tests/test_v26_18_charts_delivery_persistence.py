...
from aibl.studio_core.chart_catalog import CHART_TITLES
from aibl.studio_core.html_export import build_dynamic_html
from aibl.integrations.daily_email import build_email_html
from aibl.warehouse.sqlite_store import Warehouse, warehouse_from_settings
...
def run():
    ...
    m=re.search(r"const DATA=(.*?);\s*const COL_INDEX=",html,re.S)
    assert m and len(json.loads(m.group(1)))==3000, "embedded payload must really contain all 3000 rows"
    ...
    assert "نقطه شروع فرآیند" in html and "توزیع مرحله فعلی" in html
    assert len(CHART_TITLES) >= 15, "shared chart catalog must provide real choice"
    from aibl.studio_core.chart_catalog import CHART_SPECS
