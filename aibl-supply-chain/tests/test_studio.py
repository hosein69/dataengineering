...
def run():
    ...
    with tempfile.TemporaryDirectory() as td:
        p=Path(td)/'custom.xlsx'; out=build_custom_excel(d,p,['kpi','criticality','expert','table'],'2026-09-07',selected_fields=['KEY_MATERIAL','CANONICAL_EXPERT','CANONICAL_ORDER']); import openpyxl; wb=openpyxl.load_workbook(p); check('Excel سفارشی ساخته می‌شود',p.exists() and 'خلاصه مدیریتی' in wb.sheetnames and 'بحرانی بودن' in wb.sheetnames and 'بار کارشناسان' in wb.sheetnames and 'داده زنده' in wb.sheetnames)
