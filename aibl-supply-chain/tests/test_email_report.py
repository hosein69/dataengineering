...
def run():
 d=date(2026,8,31); p=daily_paths(d); check("پوشه تاریخ‌دار",p["folder"].name=="2026-08-31"); check("نام HTML تاریخ‌دار",p["report_html"].name=="2026-08-31_AIBL_Interactive_Report.html")
 ...
 import pathlib
 h=build_email_html(d,df,[],p["report_html"]); check("نام HTML در ایمیل", "AIBL_Interactive_Report.html" in h); check("علت متریال در ایمیل", "M1: بحرانی" in h); check("دعوت به HTML", "چرا فایل HTML را باز کنیم؟" in h and "Excel" in h and "PDF" in h); check("فونت ایمیل IRANSans است", "IRANSans" in h); check("بدون CSS خارجی", "fonts.googleapis.com" not in h)
 assets=pathlib.Path(p["assets"]); charts=make_email_charts(df,assets); check("نمودارهای ایمیل ساخته می‌شوند", len(charts)>=2 and all(x.exists() for x in charts)); check("عنوان‌های نمودار از کاتالوگ فارسی می‌آیند", "وضعیت بحرانی متریال — توزیع" in open("aibl/studio_core/designs.py",encoding="utf-8").read())
