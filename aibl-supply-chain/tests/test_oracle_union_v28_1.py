...
class OracleUnionTests(unittest.TestCase):
    ...
    def test_overlapping_part_is_filled_per_field(self):
        ...
        self.assertEqual(row["ORC_SUPPLY_GROUP"], "گروه از شیت دو")
        self.assertIn("+", row["ORC_SOURCE_SHEET"])

    def test_duplicate_inside_one_sheet_still_raises(self):
        a = sheet([{"شماره فني": "D1", "موجودي انبار ايران خودرو": 1,
                    "موجودي انبار ساپکو": 1, "نياز روزانه قطعات": 1},
                   {"شماره فني": "D1", "موجودي انبار ايران خودرو": 2,
                    "موجودي انبار ساپکو": 2, "نياز روزانه قطعات": 2}])
        with self.assertRaises(ValueError):
            OracleAdapter().transform({"1": a})
